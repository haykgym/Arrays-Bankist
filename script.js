...
const calcPrintBalance = function (acc) {
  acc.balance = acc.movements.reduce(function (aggr, val) {
    return aggr + val;
  }, 0);
...
//   both.forEach(function (val, i) {
//     if (val >= 3)
//       console.log(`Dog number ${i + 1} is adult, and ${val} years old`);
...
//   const average =
//     adults.reduce((aggr, val, i, arr) => aggr + val) / adults.length;
//   return average;
// };
...
//     .filter(val => val >= 18)
//     .reduce((aggr, val, i, arr) => aggr + val / arr.length, 0);
...
//1.
dogs.forEach(
  dog => (dog.recomendedFood = Math.trunc(Math.pow(dog.weight, 0.75) * 28))
);
console.log(dogs);
...
const copy = dogs.slice().sort((a, b) => a.recomendedFood - b.recomendedFood);
console.log(copy);
